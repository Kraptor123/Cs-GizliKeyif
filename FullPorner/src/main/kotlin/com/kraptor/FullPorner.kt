...
class FullPorner : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val videoID          = Regex("""var id = "(.+?)"""").find(iframeDocument.html())?.groupValues?.get(1)?.reversed()
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        val videoID        = Regex("""var id = "(.+?)"""").find(iframeDocument.html())?.groupValues?.getOrNull(1)
            ?.reversed()
        Log.d("kraptor_$name","videoID = $videoID")

        val extlinkList    = mutableListOf<ExtractorLink>()

        if (videoID != null) {
            val pornTrexDocument = app.get("https://www.porntrex.com/embed/${videoID}").document
            Log.d("kraptor_$name","pornTrexDocument = $pornTrexDocument")
            ...
            Log.d("kraptor_$name","videoUrls = $videoUrls")
            ...
            videoUrls.forEach { videoUrl ->
                extlinkList.add(
                    newExtractorLink(
                        ...
                        type    = INFER_TYPE,
                        {
                        this.referer = "https://www.porntrex.com/"
